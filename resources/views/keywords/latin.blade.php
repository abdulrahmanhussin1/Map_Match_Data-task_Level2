@extends('layouts.app')
@section('title')
    Keywords Latin Data Page
@endsection
@section('content')
    <main id="main" class="main">

        <div class="pagetitle">
            <h1>Keywords Latin Data Page </h1>
            <nav>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('home') }}">Home</a></li>
                    <li class="breadcrumb-item">Data</li>
                    <li class="breadcrumb-item active">Keywords Latin Data Page</li>
                </ol>
            </nav>
        </div><!-- End Page Title -->
        ...
        <section class="section">
            <div class="row">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title">Keywords Latin Data Table</h5>
                        <!-- Basic Modal -->
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title">Add Latin Keywods</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                                            aria-label="Close"></button>
                                    </div>
                                    <form action="{{ route('storeLatKeywords') }}" method="POST">
                                        @csrf
                                        <div class="modal-body">
                                            <div class="row mb-3">
                                                <div class="col-sm-10">
                                                    <textarea class="form-control" name="keyword_lat" placeholder="Add your Main Data" style="height: 100px" required>{{ old('keyword_lat') }}</textarea>
                                                </div>
                                            </div>
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                @foreach ($keywordsLat as $keyword)
                                    <form action="{{ route('mapKeywordLat') }}" method="Post">
                                        @csrf
                                        <tr>
                                            <th scope="row">{{ $loop->iteration }}</th>
                                            <td>{{ $keyword->id }}</td>
                                            <td>{{ $keyword->keyword_lat }}</td>
                                            <td>
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                @foreach ($keywordsLatMapped as $keywordMpd)
                                    <tr>
                                        <th scope="row">{{ $loop->iteration }}</th>
                                        <td>{{ $keywordMpd->id }}</td>
                                        <td>{{ $keywordMpd->keyword_lat }}</td>
